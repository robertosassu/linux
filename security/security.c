...
/**
 * security_xattr_compact - check xattrs and make array contiguous
 ...
 */
static int security_xattr_compact(struct xattr *xattrs, int num_xattrs,
				 ...
				 const initxattrs initxattrs, void *fs_data)
{
	...
	hlist_for_each_entry(P, &security_hook_heads.inode_init_security,
		...
		if (ret == -EOPNOTSUPP)
			continue;
		/*
		 * As the portion of new_xattrs touched by LSMs is not known
		 * outside, and providing such information would increase the
		 * code complexity, define instead as an approximate interval
		 * for compaction from the last compacted xattr to the end of
		 * new_xattrs.
		 */
		ret = security_xattr_compact(new_xattrs,
					     blob_sizes.lbs_xattr_count,
					     &num_filled_xattrs);
		if (ret < 0) {
			ret = -ENOMEM;
