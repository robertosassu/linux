...
{
	struct evm_iint_cache *iint = evm_iint_inode(d_backing_inode(dentry));
...
{
	struct evm_iint_cache *iint = evm_iint_inode(file_inode(file));

	if (atomic_read(&file_inode(file)->i_writecount) == 1)
