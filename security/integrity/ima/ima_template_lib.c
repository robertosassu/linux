					 ...
					 bool get_uid)
{
	...
	if (get_uid) {
		uid = i_uid_read(file_inode(event_data->file));
	} else {
		gid = i_gid_read(file_inode(event_data->file));
		data_ptr = (char *)&gid;
	}
...
/*
 *  ima_eventinodeevmxattrs_init - include EVM protected xattr values as part of
 *  the template data
 */
int ima_eventinodeevmxattrs_init_common(struct ima_event_data *event_data,
					struct ima_field_data *field_data,
					char type)
...
int ima_eventinodeevmxattrs_n_init(struct ima_event_data *event_data,
				   struct ima_field_data *field_data)
{
	return ima_eventinodeevmxattrs_init_common(event_data, field_data, 'n');
}

int ima_eventinodeevmxattrs_l_init(struct ima_event_data *event_data,
				   struct ima_field_data *field_data)
{
	return ima_eventinodeevmxattrs_init_common(event_data, field_data, 'l');
}

int ima_eventinodeevmxattrs_v_init(struct ima_event_data *event_data,
				   struct ima_field_data *field_data)
{
	return ima_eventinodeevmxattrs_init_common(event_data, field_data, 'v');
}
