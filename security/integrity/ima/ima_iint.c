 ...
 */
static inline void ima_iint_lock_lockdep_annotate(struct inode *inode)
{
...
int ima_inode_alloc_security(struct inode *inode)
{
	...
	ima_iint_lock_lockdep_annotate(inode);
...
void ima_inode_free_rcu(void *inode_security)
{
	...
	iint_lock = ima_iint_lock_security(inode_security);
