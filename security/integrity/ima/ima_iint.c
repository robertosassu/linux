 ...
 */
static inline void ima_iint_lockdep_annotate(struct inode *inode)
{
#ifdef CONFIG_LOCKDEP
	static struct lock_class_key ima_iint_lock_mutex_key[IMA_MAX_NESTING];
	struct ima_iint_cache_lock *iint_lock = inode->i_security + ima_blob_sizes.lbs_inode;
	...
	lockdep_set_class(&iint_lock->mutex, &ima_iint_lock_mutex_key[depth]);
#endif
}
				 ...
				 struct inode *inode)
{
	...
	ima_iint_lockdep_annotate(inode);
}
 ...
 * @inode: Pointer to the inode
 *
 * Initialize the mutex in the ima_iint_cache_lock structure.
 *
 * Return: Zero.
 */
...
{
	struct ima_iint_cache_lock *iint_lock = inode_security + ima_blob_sizes.lbs_inode;
