...
static void codegen_preload_free(struct bpf_object *obj, const char *obj_name)
{
	...
	codegen("\
		...
		\n\
			%1$s__destroy(skel);				    \n\
		\n\
			bpf_preload_set_ops(\"%2$s\", THIS_MODULE, NULL);   \n\
		}							    \n\
...
static int do_module(int argc, char **argv)
{
	...
	if (stat(obj_file, &st)) {
		p_err("failed to stat() %s: %s", obj_file, strerror(errno));
		return -1;
	}
	file_sz = st.st_size;
	mmap_sz = roundup(file_sz, sysconf(_SC_PAGE_SIZE));
	...
	if (fd < 0) {
		p_err("failed to open() %s: %s", obj_file, strerror(errno));
		return -1;
	}
	obj_data = mmap(NULL, mmap_sz, PROT_READ, MAP_PRIVATE, fd, 0);
	if (obj_data == MAP_FAILED) {
		obj_data = NULL;
		p_err("failed to mmap() %s: %s", obj_file, strerror(errno));
		goto out;
	}
	if (obj_name[0] == '\0')
		get_obj_name(obj_name, obj_file);
	opts.object_name = obj_name;
	if (verifier_logs)
		/* log_level1 + log_level2 + stats, but not stable UAPI */
		opts.kernel_log_level = 1 + 2 + 4;
...
static int do_help(int argc, char **argv)
{
	...
	fprintf(stderr,
		...
		"       %1$s %2$s module OBJ_FILE SKELETON_FILE [name OBJECT_NAME]\n"
		"       %1$s %2$s help\n"
		"\n"
		"       " HELP_SPEC_OPTIONS " |\n"
		"                    {-L|--use-loader} }\n"
