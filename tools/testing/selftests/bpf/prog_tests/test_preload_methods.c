...
void test_test_preload_methods(void)
{
	...
	snprintf(path, sizeof(path), "%s/preload_methods_lskel",
		 bpf_mntpoint);
	...
	snprintf(path, sizeof(path),
		 "%s/preload_methods_lskel/dump_bpf_map", bpf_mntpoint);

	err = stat(path, &st);
	if (CHECK(err, "stat", "cannot find %s\n", path))
		goto out_unmount;

	snprintf(path, sizeof(path), "%s/preload_methods_lskel/ringbuf",
		 bpf_mntpoint);
